...
### Keelung 
一个基于 Haskell 的 ZK 开发的工具。 得益于 Haskell 强大的函数式编程能力，你可以通过基础的内置数据类型，复合成复杂的数据结构。目前基隆的默认后端使用的是 Aurora，开发者正在支持 Groth16 和 PLONK。0.21 版本已经支持了绝大部分的算法，比较，位操作。0.22 版本将支持 slicing 和 joining。喜欢 Haskell的同学可以尝试一下。
